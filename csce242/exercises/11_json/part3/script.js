...
const showDrinks = async () => {
    ...
    if (drinks) {
        drinks.forEach((drinkItem) => {
            document.getElementById("dunk-picks").append(getDrinkSection(drinkItem));
        });
    } else {
...
const showTequilaDrinks = async () => {
    ...
    if (drinks) {
        drinks.forEach((drinkItem) => {
            document.getElementById("yeezy-picks").append(getDrinkSection(drinkItem));
        });
    } else {
...
const showMargaritaDrinks = async () => {
    const data = await getMargarita();
    const drinks = data.drinks; // Ensure that data.drinks exists

    if (drinks) {
        drinks.forEach((drinkItem) => {
            document.getElementById("jordan-picks").append(getDrinkSection(drinkItem));
        });
    } else {
        console.log("No tequila drinks found");
    }
...
const getMargaritaSection = (drinkItem) => {
    const section = document.createElement("section");
    section.classList.add("top-picks");
